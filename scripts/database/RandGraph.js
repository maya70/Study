(function($P){
	...
	$P.RandGraph = $P.defineClass(
		$P.BubbleBase,
		function RandGraph(config) {
			if (!(this instanceof RandGraph)) {return new RandGraph(config);}
			this.class = 'RandGraph';
			...
			if (this.dataFile) {this.dataLabel = this.dataFile.name;}
			...
			config.name = "Random"   // used to be Gallus after config._species
			    //config.name
			...
			return this;},
		{
			...
			createSvg: function(config, suppressPropogate) {
				...
				if (!this.dataType) {this.dataType = 'Random';}
				...
				this.svg = new $P.D3RandGraph(actual_config);
				this.svg.init();
