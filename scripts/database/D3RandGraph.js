(function($P){
	...
	$P.D3RandGraph = $P.defineClass(
		$P.HtmlObject,
		function D3RandGraph(config) {
			if (!(this instanceof D3RandGraph)) {return new D3RandGraph(config);}
			...
			this.name = config.name || '';  // instead of 'human' we must set this to 'random'
